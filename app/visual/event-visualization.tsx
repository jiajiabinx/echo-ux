  ...
  useEffect(() => {
    ...
    if (events.length > 0) {
      events.forEach((event, index) => {
        ...
        const size = isActive ? 1.2 : 0.8; // Increase overall size
        ...
        const material = new THREE.MeshStandardMaterial({
          ...
          transparent: true,
          opacity: isActive ? 1 : 0.9, // Increase opacity
          emissive: getEventColor(event.event_type),
          emissiveIntensity: isActive ? 2.5 : 1.8, // Increase emissive intensity
    ...
    <div className="relative h-full w-full">
      ...
      <div className="absolute bottom-4 right-4 bg-black/70 p-3 rounded text-xs max-w-xs">
        ...
        <ul className="list-disc pl-4 space-y-1 text-white/80">
          ...
          <li>click on a point: view event details</li>
        </ul>
      </div>
